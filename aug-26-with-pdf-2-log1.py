...
def format_root_cause_display(faulty_pod_details):
    """Enhanced formatting for console display with restart loop indicators"""
    ...
    # Prioritize restart loops in display
    ...
    for pod_detail in restart_loop_details[:2]:  # Show up to 2 restart loops
        root_cause = pod_detail.get('root_cause', 'Unknown restart issue')
        pod_name = pod_detail['name']
        
        # Extract the most important part for console display
        if len(root_cause) > 80:
            # Try to find the key error information
            if '🔍' in root_cause:
...
def format_root_cause_for_pdf(faulty_pod_details):
    """Enhanced formatting for PDF with detailed restart loop information"""
    if not faulty_pod_details:
        return "0: None"
    ...
    # For PDF, include comprehensive root cause analysis
    display_parts = []
...
def check_health_endpoints(endpoints, namespace):
    """Check health endpoints with enhanced restart loop reporting"""
    if not endpoints:
        return [], 0
    ...
    for i, ep in enumerate(endpoints, 1):
        ...
        try:
            ...
            # Enhanced root cause display with restart loop emphasis
            ...
            # Show detailed restart loop analysis if present
            if restart_loop_pods:
                ...
                for pod_detail in faulty_pod_details:
                    if pod_detail.get('is_restart_loop', False):
                        ...
                        print(f"    └─ 🔍 {pod_name}:")
                        print(f"       {root_cause}")
            elif faulty_pod_details and not health_status.startswith('🟢'):
                # Show other fault analysis for non-restart issues
...
def check_basic_connectivity(basic_endpoints, namespace):
    """Check basic connectivity for services without health endpoints with enhanced restart loop reporting"""
    ...
    for i, ep in enumerate(basic_endpoints, 1):
        ...
        try:
            ...
            # Enhanced root cause display with restart loop indicators
            root_cause_display = format_root_cause_for_pdf(faulty_pod_details)
            ...
            if restart_loop_pods:
                ...
                for pod_detail in faulty_pod_details:
                    if pod_detail.get('is_restart_loop', False):
                        ...
                        print(f"    └─ 🔍 {pod_name}:")
                        print(f"       {root_cause}")
            elif faulty_pod_details and not connectivity_status.startswith('🟢'):
                # Show other fault analysis for non-restart issues
                for pod_detail in faulty_pod_details[:1]:  # Show first faulty pod in detail
                    root_cause = pod_detail.get('root_cause')
                    if root_cause:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="K8s health check monitor with intelligent restart loop analysis")
