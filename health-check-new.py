...
def get_health_check_endpoints(namespace):
    ...
    for svc in services.items:
        ...
        if not selector:
            logger.info(f"Service '{svc_name}' has no selector.")
            continue
        ...
        if not matched_pod:
            logger.info(f"No pod found for service '{svc_name}'")
            continue
        ...
        if service_has_health_probe and health_path:
            # Get ingress rules for this service
                            ...
                            if rule.http and rule.http.paths:
                                for path in rule.http.paths:
                                    if (path.backend.service and 
                                        ...
                                        logger.info(f"Health Endpoint for service '{svc_name}': {endpoint}")
        else:
            logger.info(f"Service '{svc_name}' does not have a valid health check endpoint")
...
if __name__ == "__main__":
    ...
    # Print summary
    print(f"\nFound {len(endpoints)} valid health check endpoints:")
    for ep in endpoints:
        print(f"  {ep['service']}: {ep['endpoint']}")
