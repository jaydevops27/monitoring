...
class HealthCheckReport:
    ...
    def _create_tmobile_stats_table(self, health_results, healthy_count, basic_results,
        ...
        # Count unique services (since we now have multiple endpoints per service)
        unique_services = set()
...
def get_health_check_endpoints(namespace):
    ...
    for svc in services.items:
        ...
        if health_path and ingress_endpoints:
            # Service has both health probe and ingress(es) - create entry for each DNS endpoint
            for i, ingress_endpoint in enumerate(ingress_endpoints):
                # Create unique identifier for each DNS endpoint
                if len(ingress_endpoints) > 1:
                    display_name = f"{svc_name} [{ingress_endpoint.replace('https://', '')}]"
                else:
                ...
                health_endpoints.append({
                    ...
                    'dns_host': ingress_endpoint.replace('https://', ''),
            ...
            # Service has ingress(es) but no health probe - test basic connectivity for each DNS endpoint
            for i, ingress_endpoint in enumerate(ingress_endpoints):
                # Create unique identifier for each DNS endpoint
                if len(ingress_endpoints) > 1:
                    display_name = f"{svc_name} [{ingress_endpoint.replace('https://', '')}]"
...
def check_health_endpoints(endpoints, namespace):
    """Check health endpoints with conditional restart loop reporting"""
    if not endpoints:
        return [], 0
    ...
    for i, ep in enumerate(endpoints, 1):
        ...
        dns_host = ep.get('dns_host', 'unknown')
        
        # Adjust display width for longer service names with DNS
        max_service_len = max(20, len(service_name))
        print(f"[{i}/{len(endpoints)}] {service_name:<{max_service_len}}", end=' ')
        ...
        try:
            ...
            root_cause_display_console = format_root_cause_display(faulty_pod_details, service_is_healthy)
            ...
            # Only show detailed restart loop analysis if service is NOT healthy
            if not service_is_healthy and restart_loop_pods:
                ...
                # Service is healthy but had restarts - show brief summary
                ...
                # Service is not healthy - show fault analysis
            ...
            root_cause_display = format_root_cause_for_pdf(faulty_pod_details, service_is_healthy=False)
...
def check_basic_connectivity(basic_endpoints, namespace):
    """Check basic connectivity for services without health endpoints with conditional restart loop reporting"""
    ...
    for i, ep in enumerate(basic_endpoints, 1):
        ...
        # Adjust display width for longer service names with DNS
        max_service_len = max(20, len(service_name))
        print(f"[{i}/{len(basic_endpoints)}] {service_name:<{max_service_len}}", end=' ')
        ...
        try:
            ...
            # Check if service is actually reachable based on response content and status
            is_service_reachable = False
            response_indicators = []
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="K8s health check monitor with intelligent restart loop analysis and multi-DNS support")
