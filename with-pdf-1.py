...
class HealthCheckReport:
    ...
    def generate_pdf(self, filename=None):
        ...
        if self.report_data['health_results']:
            story.append(Paragraph("Health Check Results", heading_style))
        ...
        if self.report_data['basic_results']:
            story.append(Paragraph("Basic Connectivity Results", heading_style))
    ...
    def _generate_summary(self):
        ...
        suspended_services = len(stats.get('suspended_services', []))
        ...
        summary = f"""
        Total Services Monitored: {total_services}
        Healthy Services: {healthy_services} ({health_rate:.1f}%)
        Suspended Services: {suspended_services}
    ...
    def _create_stats_table(self):
        ...
        data = [
            ...
            ['Suspended Services', str(len(stats.get('suspended_services', [])))],
            ['Services without Health Probe', str(len(stats.get('no_health_probe', [])))],
            ['Services without Ingress', str(len(stats.get('no_ingress', [])))]
        ]
    ...
    def _create_results_table(self, results):
        ...
        clean_data = [['Service', 'Status', 'Total Pods', 'Faulty Pods']]
        ...
        table = Table(clean_data, colWidths=[2.5*inch, 1.5*inch, 1*inch, 2*inch])
    ...
    def add_health_results(self, results, healthy_count, total_count):
        ...
        self.report_data['statistics']['total_services'] = total_count
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.output_format in ['pdf', 'both'] and report:
            ...
            report.add_health_results(health_results, healthy_count, total_services)
            report.add_basic_results(basic_results)
            report.add_statistics({
                'suspended_services': suspended_services,
                'no_selector': services_no_selector,
                'no_health_probe': services_no_health_probe,
                'no_ingress': services_no_ingress,
                'services_with_issues': total_services - healthy_count
            })
