...
class HealthCheckReport:
    ...
    def _create_tmobile_results_table(self, results):
        ...
        clean_data = [['Service', 'Status', 'Pods', 'Restart Count / Root Cause / DNS Issues']]
        ...
        for row in results:
            ...
            for i, cell in enumerate(row):
                ...
                if i == 0:  # Service name - use Paragraph for consistent formatting
                    ...
                    clean_cell = Paragraph(clean_cell, content_style)
                elif i == 3:  # Restart count or root cause analysis - structured format
                    formatted_analysis = self._format_restart_or_root_cause_for_pdf(clean_cell)
    ...
    def _format_restart_or_root_cause_for_pdf(self, analysis_text):
        """Format restart count or root cause analysis for PDF with consistent professional formatting"""
        # Handle simple restart count format
        if 'restarts' in analysis_text and 'restart' in analysis_text.lower():
...
def get_pod_stats(pods, selector, v1, namespace):
    ...
    for pod in matching_pods:
        ...
        # Add pod to details if faulty OR has restarts (for UP services restart tracking)
        if is_faulty or restart_count > 0:
            if is_faulty:
                # Get intelligent fault analysis only for faulty pods
                reasons, root_cause = analyze_pod_fault(pod, v1, namespace)
            else:
                # For non-faulty pods with restarts, just track restart count
                reasons = [f"Restarts: {restart_count}"]
                root_cause = None
                    ...
                    if rule.host and rule.http and rule.http.paths:
                        for path in rule.http.paths:
                            if path.backend.service and path.backend.service.name == svc_name:
                                ingress_base = f"https://{rule.host}"
                                if ingress_base not in ingress_endpoints:
                                    ingress_endpoints.append(ingress_base)
        
        if health_path and ingress_endpoints:
            # Service has both health probe and ingress(es)
            # Create health endpoint and multiple connectivity endpoints
            health_endpoint = f"{ingress_endpoints[0]}{health_path}"  # Use first ingress for health
            connectivity_endpoints = ingress_endpoints  # All ingresses for connectivity testing
            ...
            # Service has ingress(es) but no health probe - test basic connectivity only
            basic_endpoints[svc_name] = {
                'service': svc_name,
                'connectivity_endpoints': ingress_endpoints,
                'total_pods': total_pods,
                'faulty_pod_details': faulty_pod_details
            }
...
def format_restart_count_display(faulty_pod_details):
    """Format restart count for UP services (no detailed analysis needed)"""
    if not faulty_pod_details:
        return f"{C.G}0 restarts{C.E}"
    
    # For UP services, just show restart counts
    total_restarts = 0
    restart_details = []
    
    for pod_detail in faulty_pod_details:
        pod_name = pod_detail['name']
        restart_count = pod_detail.get('restart_count', 0)
        
        if restart_count > 0:
            total_restarts += restart_count
            restart_details.append(f"{pod_name}({restart_count})")
    
    if total_restarts > 0:
        if len(restart_details) > 2:
            shown = restart_details[:2]
            remaining = len(restart_details) - 2
            return f"{C.Y}{total_restarts} restarts: {', '.join(shown)}... (+{remaining} more){C.E}"
        else:
            return f"{C.Y}{total_restarts} restarts: {', '.join(restart_details)}{C.E}"
    else:
        return f"{C.G}0 restarts{C.E}"

def format_restart_count_for_pdf(faulty_pod_details):
    """Format restart count for PDF (UP services)"""
    if not faulty_pod_details:
        return "0 restarts"
    ...
    if total_restarts > 0:
        return f"{total_restarts} restarts: " + ', '.join(restart_details)
    else:
        return "0 restarts"

def format_root_cause_display(faulty_pod_details):
    """Format root cause analysis for console display (ERROR cases only)"""
    if not faulty_pod_details:
...
def check_service_health_and_connectivity(service_info, namespace):
    """
    Check service health with priority-based logic:
    1. FIRST PRIORITY: Health endpoint status (authoritative)
    2. SECOND PRIORITY: DNS/connectivity issues (supplementary)
    
    Returns: (health_status, connectivity_issues, is_healthy)
    """
    service_name = service_info['service']
    health_endpoint = service_info.get('health_endpoint')
    connectivity_endpoints = service_info.get('connectivity_endpoints', [])
    
    # STEP 1: Check health endpoint (PRIORITY 1)
    health_status = None
    health_error = None
    
    if health_endpoint:
        try:
            print(f"  🏥 Health: {health_endpoint}", end=' ')
            response = requests.get(health_endpoint, timeout=8, verify=False, headers={'Accept': 'application/json'})
            ...
            if response.status_code == 200:
                try:
                    ...
                    if 'status' in health_data:
                        ...
                        health_status = 'UP' if (pong_value is True or str(pong_value).lower() == 'true') else 'DOWN'
                    else:
                        health_status = 'UP'  # JSON response with 200 OK
                    ...
                    if any(phrase in response_text for phrase in [
                        ...
                        'cannot get /', 'not found', 'method not allowed'
                    ]):
                        health_status = 'UP'  # Service responding with structured error
                    else:
                        health_status = 'UP'  # Non-JSON but 200 OK
                
                print(f"✅ {health_status}")
            else:
                health_error = f"Health endpoint HTTP {response.status_code}"
                print(f"❌ HTTP {response.status_code}")
                
        except requests.exceptions.Timeout:
            health_error = "Health endpoint timeout"
            print(f"⏱️  TIMEOUT")
        except requests.exceptions.ConnectionError:
            health_error = "Health endpoint unreachable"
            print(f"🚫 UNREACHABLE")
        except Exception as e:
            health_error = f"Health endpoint error: {str(e)}"
            print(f"💥 ERROR")
    
    # STEP 2: Check connectivity endpoints (PRIORITY 2)
    connectivity_issues = []
    
    if connectivity_endpoints:
        print(f"  🌐 DNS/Connectivity checks:")
        for endpoint in connectivity_endpoints:
            try:
                print(f"    - {endpoint}", end=' ')
                response = requests.get(endpoint, timeout=5, verify=False, allow_redirects=True)
                
                if response.status_code in [200, 301, 302, 403]:
                    print(f"✅ ACCESSIBLE")
                elif response.status_code == 404:
                    # Check if it's a structured error (service up, just no route)
                    response_text = response.text.lower()
                    if any(phrase in response_text for phrase in [
                        'whitelabel error page', 'cannot get /', 'not found',
                        'spring boot', 'express', 'nginx'
                    ]):
                        print(f"✅ ACCESSIBLE (structured 404)")
                    else:
                        connectivity_issues.append(f"{endpoint}: HTTP 404")
                        print(f"⚠️  HTTP 404")
                else:
                    connectivity_issues.append(f"{endpoint}: HTTP {response.status_code}")
                    print(f"❌ HTTP {response.status_code}")
                    
            except requests.exceptions.Timeout:
                connectivity_issues.append(f"{endpoint}: Timeout")
                print(f"⏱️  TIMEOUT")
            except requests.exceptions.ConnectionError:
                connectivity_issues.append(f"{endpoint}: Unreachable")
                print(f"🚫 UNREACHABLE")
            except Exception as e:
                connectivity_issues.append(f"{endpoint}: Error - {str(e)}")
                print(f"💥 ERROR")
    
    # STEP 3: Determine final status based on PRIORITY
    if health_status == 'UP':
        # Health endpoint is UP - service is healthy regardless of DNS issues
        is_healthy = True
        final_status = '🟢 UP'
        if connectivity_issues:
            # Add DNS issues as supplementary info
            dns_info = f"DNS Issues: {'; '.join(connectivity_issues)}"
        else:
            dns_info = None
    elif health_error:
        # Health endpoint failed - service is unhealthy
        is_healthy = False
        final_status = f'🔴 {health_error}'
        dns_info = f"Connectivity: {'; '.join(connectivity_issues)}" if connectivity_issues else None
    else:
        # No health endpoint - fall back to connectivity status
        if connectivity_issues:
            is_healthy = False
            final_status = f'🔴 Connectivity Issues'
            dns_info = '; '.join(connectivity_issues)
        else:
            is_healthy = True
            final_status = '🟢 ACCESSIBLE'
            dns_info = None
    
    return final_status, dns_info, is_healthy

def check_health_endpoints(endpoints, namespace):
    """Check health endpoints with priority-based logic: Health first, then connectivity"""
    if not endpoints:
        return [], 0
    
    print(f"\n{C.B}{C.C}🏥 Priority Health Check: {namespace} ({len(endpoints)} services){C.E}")
    print(f"{C.C}{'─' * 80}{C.E}")
    
    results = []
    healthy_count = 0
    
    for i, service_info in enumerate(endpoints, 1):
        service_name = service_info['service']
        total_pods = service_info['total_pods']
        faulty_pod_details = service_info['faulty_pod_details']
        
        print(f"[{i}/{len(endpoints)}] {service_name:<20}")
        
        # Use priority-based checking
        final_status, dns_info, is_healthy = check_service_health_and_connectivity(service_info, namespace)
        
        if is_healthy:
            healthy_count += 1
            # For healthy services, show restart count + DNS issues (if any)
            restart_display = format_restart_count_for_pdf(faulty_pod_details)
            if dns_info:
                combined_info = f"{restart_display}; {dns_info}"
            else:
                combined_info = restart_display
            results.append([service_name, final_status, str(total_pods), combined_info])
        else:
            # For unhealthy services, show root cause analysis + DNS info
            root_cause_display = format_root_cause_for_pdf(faulty_pod_details)
            if dns_info:
                combined_info = f"{root_cause_display}; {dns_info}"
            else:
                combined_info = root_cause_display
            results.append([service_name, final_status, str(total_pods), combined_info])
            
            # Show detailed pod analysis for unhealthy services
            if faulty_pod_details:
                for pod_detail in faulty_pod_details[:1]:
                    root_cause = pod_detail.get('root_cause')
                    if root_cause:
                        print(f"    └─ 🔍 Pod Issue: {pod_detail['name']}: {root_cause}")
    
    return results, healthy_count

def check_basic_connectivity(basic_endpoints, namespace):
    """Check basic connectivity for services without health endpoints (multiple DNS support)"""
    ...
    for i, service_info in enumerate(basic_endpoints, 1):
        service_name = service_info['service']
        connectivity_endpoints = service_info.get('connectivity_endpoints', [])
        total_pods = service_info['total_pods']
        faulty_pod_details = service_info['faulty_pod_details']
        
        print(f"[{i}/{len(basic_endpoints)}] {service_name:<20}")
        
        # Check all connectivity endpoints
        connectivity_issues = []
        accessible_count = 0
        
        print(f"  🌐 Testing {len(connectivity_endpoints)} endpoint(s):")
        for endpoint in connectivity_endpoints:
            try:
                print(f"    - {endpoint}", end=' ')
                response = requests.get(endpoint, timeout=5, verify=False, allow_redirects=True)
                ...
                if response.status_code in [200, 301, 302, 403]:
                    ...
                    # Check if it's a structured error (service up, just no specific route)
                    response_text = response.text.lower()
                    if any(phrase in response_text for phrase in [
                        'whitelabel error page', 'cannot get /', 'not found',
                        'spring boot', 'express', 'nginx', 'tomcat', 'jetty'
                    ]):
                        print(f"✅ ACCESSIBLE (structured 404)")
                        accessible_count += 1
                    else:
        ...
        # Determine final status based on accessibility
        if accessible_count > 0:
            # At least one endpoint is accessible
            connectivity_status = '🟢 ACCESSIBLE'
            restart_display = format_restart_count_for_pdf(faulty_pod_details)
            if connectivity_issues:
                combined_info = f"{restart_display}; DNS Issues: {'; '.join(connectivity_issues)}"
            else:
                combined_info = restart_display
        else:
            ...
            root_cause_display = format_root_cause_for_pdf(faulty_pod_details)
            combined_info = f"{root_cause_display}; Connectivity: {'; '.join(connectivity_issues)}"
            
            # Show detailed pod analysis for unreachable services
            if faulty_pod_details:
                for pod_detail in faulty_pod_details[:1]:
                    root_cause = pod_detail.get('root_cause')
                    if root_cause:
        ...
        results.append([service_name, connectivity_status, str(total_pods), combined_info])
...
def print_results(health_results, healthy_count, basic_results, services_no_selector, services_no_health_probe, services_no_ingress, suspended_services):
    ...
    if health_results:
        ...
        print(f"\n{C.B}📊 Priority Health Check Results{C.E}")
        print(tabulate(health_results, headers=['Service', 'Health Status', 'Total Pods', 'Restart Count / Root Cause / DNS Issues'], tablefmt='simple'))
        
        print(f"\n{C.B}Health Stats:{C.E} {C.G}{healthy_count}/{total_endpoints} healthy{C.E} ({success_rate:.0f}%)")
    
    # Basic connectivity results
    if basic_results:
        print(f"\n{C.B}🌐 Multi-Endpoint Connectivity Results{C.E}")
        print(tabulate(basic_results, headers=['Service', 'Connectivity Status', 'Total Pods', 'Restart Count / Root Cause / DNS Issues'], tablefmt='simple'))
    ...
    print(f"\n{C.B}🎯 Priority-Based Monitoring Summary:{C.E}")
    print(f"• Health endpoints are checked first (authoritative status)")
    print(f"• DNS/connectivity issues are tracked separately")
    print(f"• Services can be UP even with some DNS problems")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="K8s health check monitor with priority-based logic. Health endpoints take priority over DNS issues. Shows restart counts for UP services, detailed root cause analysis for ERROR services.")
